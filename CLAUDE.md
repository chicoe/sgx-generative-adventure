...
**Placeholder-only content.** This is scaffolding: the client supplies ALL real story, text, images, and assets later (via the editor). Keep every in-repo text and image an obvious stand-in (`Scene A`, `placeholder behaviour`, `[ placeholder intro ]`, labeled "missing asset" art) — never invent narrative, characters, or finished-looking art. The cruise-ship premise in `SPEC.md` only illustrates the mechanic; do not ship it as content. Demo content lives in `src/lib/game/placeholderBuild.ts` (its scenes have NO layers on purpose — the runtime renders NO SIGNAL static for art-less scenes; the old `static/placeholder/` SVG art was deleted). The placeholder build exists for tests + editor seeding only: **`/play` never runs it** — if the live build can't load (network error / nothing published), `Game.svelte` shows a discrete black "please restart" fail page instead, and recovers automatically via the config/current watcher when a publish arrives. (Exception: the off-black + amber 80s/aerospace **CRT interface skin** — `app.css` palette, self-hosted Monaspace Neon/Krypton fonts in `static/fonts/`, `/play` scanlines + phosphor glow — is **owner-directed and intentional**; the placeholder rule covers scene content, not the app chrome.)
...
- **M3:** Firestore content model — `src/lib/content/`: `schema.ts` (zod for all content, drift-checked against `placeholderBuild`), `build.ts` (pure `assembleBuild` with referential-integrity checks + `serialize/deserializeBuild`), `loader.ts` (client-SDK read of `config/current`→`builds/{id}`, **falls back to `placeholderBuild`**). `/play` loads the active build on mount and **live-watches `config/current`** (`onSnapshot` in `Game.svelte`) — publishing a different version triggers a full `location.reload()` so the kiosk picks up new content + app code by itself (once-per-buildId sessionStorage guard prevents reload loops; `/testplay` opts out via `reloadOnPublish={false}`). Builds are stored as one doc with a JSON `payload` (single read). Draft layout: `draft/content` (+ `scenes`/`items`/`behaviours` subcollections).
- **M4:** the editor at `/editor` (client-only, `ssr=false`). Auth via Firebase Email/Password (`src/lib/firebase/auth.svelte.ts` reactive store); `+layout.svelte` shows a login gate then the editor. `src/lib/content/draft.ts` (draft CRUD, undefined-safe via JSON round-trip) + `publishClient.ts` (**client-side** two-step release — `saveDraftBuild` validates with `assembleBuild` and commits `builds/{id}` with an optional commit message; `setActiveBuild` flips `config/current`; gated by `firestore.rules` where editors can _create_ builds and write config, builds stay immutable). Done: **Overview page** (seed → validate&save → publish version / restore version settings), **scenes editor** (layers + **image upload** via `src/lib/firebase/storage.ts`, filter, `prompt`, onEnter effects, start/ending checkboxes; exits are on the graph, not here), **items editor**, **behaviour editor** + **live test panel**. Reusable `EffectsEditor.svelte` + `ConditionsEditor.svelte` (hasItem/flag AND-combined; or/not not surfaced yet). `SceneRenderer` resolves absolute URLs (Storage) as-is. `/api/converse` takes an inline `behaviour` (test panel) or resolves `behaviourId` from the **active build**. The **Svelte Flow scene graph** at `/editor/graph` (`@xyflow/svelte`, dark mode) **owns exits**: custom `SceneNode` shows a stacked preview of the scene's layer art (★ = start); dragging between nodes adds an exit (label defaults to the target scene's name); **clicking an edge** opens an inline editor to rename/remove that exit (Backspace also removes); node-click deep-links to the scene editor (`?scene=id`); a "new scene" control creates one; node layout is **persisted** to `graphPositions` on the `draft/content` doc (editor-only, not in the build). **Links are two-way DOORS by default** (`Exit.oneWay?` makes one directional; `Exit.requiredItems?` hard-locks a door — both directions — until the player holds all listed items; pure `availableDoors()` in `engine/graph.ts` resolves doors+locks and the runtime only offers UNLOCKED doors to the LLM). The edge editor on the graph edits label + one-way + required items (locked edges render dashed + 🔒; two-way edges have arrows on both ends). The scenes editor has a **⧉ Duplicate scene** button (full copy incl. exits, fresh ids). The **scene form no longer edits exits** (only the graph does) — note: exit `condition`s have no UI yet (they persist if present). `firestore.rules` + `storage.rules` are deployed. **Editor auth: ANY signed-in user is an editor** (`isEditor()` = `request.auth != null`) — safe only because accounts are admin-created in the console and **self-service sign-up must stay disabled** (Authentication → Settings → User actions). Editor flows need a signed-in user (can't be fully tested headlessly).
...
4. **The game reads only the active immutable build.** Content has two worlds in Firestore: mutable `draft/*` (editor working copies) and immutable `builds/{buildId}` snapshots (incl. display settings; optional commit `message` on the doc). The game reads `config/current.activeBuildId` → that build only. Two-step release: **"Validate & save"** (`saveDraftBuild`) validates the draft with zod and commits a new build; **"publish version"** (`setActiveBuild`) atomically points `config/current` at a build — also how rollback works. Editing drafts can never break the live game; **"restore version settings"** (`restoreDraftFromBuild`) rewinds the draft (content + settings, keeps graphPositions) to any build.
...
Provide `.env.example`. Keys: `GEMINI_API_KEY` (server only), `GEMINI_MODEL` (default `gemini-flash-lite`), `PUBLIC_FIREBASE_CONFIG` (public client config), `FIREBASE_ADMIN_CREDENTIALS` (server), `EDITOR_ALLOWED_EMAILS` (legacy — editor access is now "any signed-in user"; accounts are admin-created with sign-up disabled).
