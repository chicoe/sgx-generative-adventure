...
- **M2:** the LLM loop — `src/lib/llm/`: `outcome.ts` (zod response schema), `prompt.ts` (prompt builder), `adjudicate.ts` (pure outcomeId→effects resolution + deterministic fallback), and **server-only** `gemini.ts` (backend-agnostic: developer-key or Vertex/ADC via `GEMINI_BACKEND`). Endpoint `POST /api/converse` + a keyboard dialogue panel in `/play`. Pure modules unit-tested; live model verified working.
...
- **M4 (in progress):** the editor at `/editor` (client-only, `ssr=false`). Auth via Firebase Email/Password (`src/lib/firebase/auth.svelte.ts` reactive store); `+layout.svelte` shows a login gate then the editor. `src/lib/content/draft.ts` (draft CRUD) + `publishClient.ts` (**client-side** atomic publish/rollback — validates with `assembleBuild`, then batches `builds/{id}` + `config/current`; gated by `firestore.rules` where editors can _create_ builds and write config, builds stay immutable). Dashboard does seed-from-placeholder → publish → rollback. **Built but scene/item/behaviour editors + the Svelte Flow graph + image upload + behaviour test panel are still TODO.** Auth/publish need a signed-in allowlisted editor to exercise (can't be tested headlessly).

Outstanding acceptance items: M1's on-Pi kiosk verification (hardware; owner runs it); the editor's live click-through (sign in at `/editor` as an allowlisted user → seed → publish → check `/play` → rollback); the rest of the M4 authoring UI.
