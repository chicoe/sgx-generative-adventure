...
- **M2:** the LLM loop — `src/lib/llm/`: `outcome.ts` (zod response schema), `prompt.ts` (prompt builder), `adjudicate.ts` (pure outcomeId→effects resolution + deterministic fallback), and **server-only** `gemini.ts` (backend-agnostic: developer-key or Vertex/ADC via `GEMINI_BACKEND`). Endpoint `POST /api/converse` + a keyboard dialogue panel in `/play`. Pure modules unit-tested; live model verified working. **Scene context:** the converse request carries optional `sceneContext` (the scene's `prompt` field + available exits + inventory); `prompt.ts` injects it into the system instruction so the computer converses in situ and can **trigger an exit/action by selecting an authored outcome** whose effects include `goToScene` (etc.). Scene context is informational only — invariant #1 still holds (the model picks an authored `outcomeId`; it never invents effects).
